...
import struct
import collections
import openflow.ryulib.common_defs as ofproto_common
import openflow.ryulib.openflow_v13_defs as ofproto
import openflow.ryulib.openflow_exceptions as openflow_exceptions
import openflow.ryulib.ether as ether
import openflow.ryulib.mac as mac
from openflow.ryulib.utils import l2, round_up, msg_pack_into


# This will map a message class to its parser method
# We will not map versions though, since we only plan to implement 1.3
_MSG_PARSERS = {}
...
class MsgBase:
    ...
    ========= ==============================
    datapath  (CHANGED) A datapath IDENTIFIER ... not the object
    ...
    def _serialize_pre(self):
        ...
        self.buf = bytearray(ofproto.OFP_HEADER_SIZE)
    ...
    def _serialize_header(self):
        ...
        assert len(self.buf) >= ofproto.OFP_HEADER_SIZE

        self.msg_len = len(self.buf)
        if self.xid is None:
            self.xid = 0

        struct.pack_into(ofproto.OFP_HEADER_PACK_STR,
                         self.buf, 0,
...
def namedtuple(typename, fields, **kwargs):
    class _namedtuple(collections.namedtuple(typename, fields, **kwargs)):
        pass
    return _namedtuple


def _set_msg_type(msg_type):
    '''class decorator to register message type'''
...
# Generic method for parsing a message. 
...
class OFPHelloElemVersionBitmap:
    """
...
class OFPMatch:
    ...
    def parser(cls, buf, offset):
        ...
        if exc is not None:
            raise openflow_exceptions.TruncatedMessage(residue)
        return match
...
class OFPPropCommonExperimenter4ByteData:
    _PACK_STR = '!HHII'
...
class OFPPacketIn(MsgBase):
    ...
    def parser(cls, datapath, version, msg_type, msg_len, xid, buf):
        ...
        match_len = round_up(msg.match.length, 8)
...
class OFPPort(namedtuple('OFPPort', (
...
class OFPPacketOut(MsgBase):
    ...
    def from_jsondict(cls, dict_, decode_string=base64.b64decode,
                      **additional_args):
        raise NotImplementedError("Need to implement Packet")
...
class OFPFlowMod(MsgBase):
    ...
    def parser(cls, datapath, version, msg_type, msg_len, xid, buf):
        ...
        except openflow_exceptions.TruncatedMessage as e:
            msg.match = e.ofpmsg
            e.ofpmsg = msg
            raise e

        offset += round_up(msg.match.length, 8)
        ...
        try:
            ...
            msg.instructions = instructions
            raise openflow_exceptions.TruncatedMessage(buf[offset:])
        msg.instructions = instructions
...
class OFPInstructionActions(OFPInstruction):
    ...
    def serialize(self, buf, offset):
        ...
        pad_len = round_up(self.len, 8) - self.len
...
class OFPBucket:
    def __init__(self, weight=1, watch_port=ofproto.OFPP_ANY,
    ...
    def serialize(self, buf, offset):
        ...
        self.len = round_up(ofproto.OFP_BUCKET_SIZE + action_len, 8)
...
class OFPPortMod(MsgBase):
    ...
    def _serialize_body(self):
        msg_pack_into(ofproto.OFP_PORT_MOD_PACK_STR, self.buf,
                      ofproto.OFP_HEADER_SIZE,
                      self.port_no, l2.text_to_bin(self.hw_addr),
                      self.config,
...
class OFPDescStats(namedtuple('OFPDescStats', (
...
class OFPFlowStats:
    ...
    def parser(cls, buf, offset):
        ...
        match_length = round_up(flow_stats.match.length, 8)
...
class OFPAggregateStats(namedtuple('OFPAggregateStats', (
...
class OFPTableStats(namedtuple('OFPTableStats', (
...
class OFPPortStats(namedtuple('OFPPortStats', (
...
class OFPQueueStats(namedtuple('OFPQueueStats', (
...
class OFPGroupFeaturesStats(namedtuple('OFPGroupFeaturesStats',
...
class OFPMeterFeaturesStats(namedtuple('OFPMeterFeaturesStats',
...
class OFPExperimenterMultipart(namedtuple(
...
class ONFFlowMonitorRequest:
    ...
    def serialize(self):
        ...
        buf += bin_match
        pad_len = round_up(self.match_len, 8) - self.match_len
        buf += pad_len * b'\0'
        return buf

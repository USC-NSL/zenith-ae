...
from openflow.ryulib.utils import l2, ipv4, ipv6
...
class MacAddr(TypeDescr):
    size = 6
    to_user = l2.bin_to_text
    from_user = l2.text_to_bin


class IPv4Addr(TypeDescr):
    size = 4
    to_user = ipv4.bin_to_text
    from_user = ipv4.text_to_bin


class IPv6Addr(TypeDescr):
    size = 16
    to_user = ipv6.bin_to_text
    from_user = ipv6.text_to_bin
